...
const MoodHeader: React.FC<MoodHeaderProps> = ({ userName = '' }) => {
  ...
  return (
    <header className="py-12 px-4 text-center slide-up relative">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex justify-center items-center gap-3 mb-6">
          <div className="relative">
            <Heart className="h-10 w-10 text-mind-pink float" fill="#F8C8DC" />
            <Sparkles className="absolute -right-2 -top-2 h-5 w-5 text-mind-yellow animate-pulse-soft" />
          </div>
          <h1 className="text-4xl font-bold bg-gradient-calm text-transparent bg-clip-text">
            Mindful Muse
          </h1>
        </div>
        
        <h2 className="text-2xl text-slate-600 mt-4 font-medium flex items-center justify-center gap-2">
          <TimeIcon className="h-6 w-6 text-mind-yellow" />
          {timeGreeting}{userGreeting}
        </h2>
        
        <div className="max-w-md mx-auto mt-6">
          <p className="text-lg text-slate-500">Your AI companion for mental wellness and emotional support</p>
          <p className="text-md text-mind-purple/80 mt-4 italic font-light">"{getRandomAffirmation()}"</p>
        </div>
      </div>
    </header>
  );
